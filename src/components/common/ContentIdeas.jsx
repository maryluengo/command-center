...
const emptyCard = () => ({
  ...
  pillar: 'Fashion', platform: 'Instagram',
  effort: 'Quick', status: 'Just an Idea',
  files: [], links: [''], client: '',
})

export default function ContentIdeas({ storageKey, showClient = false, clients = [] }) {
  const [ideas, setIdeas] = useLocalStorage(`content-ideas-${storageKey}`, [])

  // Customizable options
  const { options: pillars }    = useCustomOptions('ideas-pillars',   DEFAULT_PILLARS)
        ...
        <div className="ideas-board">
          {filtered.map(card => (
            <div key={card.id} className="idea-card">
              <div className="idea-card-top" style={{ background: pillarColor(card.pillar, pillars) }} />
